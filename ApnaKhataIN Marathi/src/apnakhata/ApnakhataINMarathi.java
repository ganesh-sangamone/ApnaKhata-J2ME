 ...
 * @author Ganesh
 */
public class ApnakhataINMarathi extends MIDlet implements CommandListener, ItemStateListener {
    ...
    public ApnakhataINMarathi(){    
        ...
        if(check.equalsIgnoreCase("true")) {  //wait
            i = readRecords();            
            cg = new ChoiceGroup("आपली बॅंक निवडा", ChoiceGroup.EXCLUSIVE);           
            if(i == 1) {
                bankOperation();
            }
            else {
                 aa = new Alert("चेतावनी","AddBank जा आणि आपल्या बँका निवडा", null, AlertType.ALARM);
            ...
            Alert a3;
            a3 = new Alert("चेतावनी", "ApnaKhata सक्रिय करा", null, AlertType.WARNING);
    ...
    public static String[] split(String splitStr)
    {
     ...
     if (token.length() > 0) {
        temp = token.toString();
         ex.setLength(0);
         ex.append(temp.substring(temp.indexOf('*')+1,temp.lastIndexOf('*')));
         function.addElement("कॉल कस्टमर केयर".trim());
         cn = ex.toString();
         ex.setLength(0);
         token.setLength(0);
         ex.append(temp.substring(temp.indexOf('@')+1,temp.lastIndexOf('@')));
         function.addElement("वेबसाइट");    
         wb = ex.toString();
         token.setLength(0);
         ex.setLength(0);
   ...
   public String file(String filename){
     ...
     public class GUI implements CommandListener, ItemStateListener {      
        private ApnakhataINMarathi parent;
        ...
        public GUI(ApnakhataINMarathi parent, String message, String bname) {   
        
        back1 = new Command ("Back", Command.BACK, 1);
        submit1 = new Command("Submit", Command.SCREEN, 1);
        send1 = new Command("Send", Command.OK, 1);
        radioButton = new ChoiceGroup("एक पर्याय निवडा",Choice.EXCLUSIVE);
