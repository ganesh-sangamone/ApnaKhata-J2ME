...
public class AddBank extends MIDlet implements CommandListener {
    ...
    public AddBank(){       
        ...
        if(check.equalsIgnoreCase("true")) {
            cg = new ChoiceGroup("તમારી બેંક પસંદ કરો", ChoiceGroup.MULTIPLE);           
            bankOperation();
            readRecords();
        }
        else {
            Alert a3;
            a3 = new Alert("ચેતવણી", "ઉત્પાદન સક્રિય કરો", null, AlertType.WARNING);
    ...
    public void save() {
        ...
        try {
            ...
            if(cg.getSelectedFlags(picks) > 0){            
                ...
                a = new Alert("ચેતવણી", "ઓછામાં ઓછી એક બેંક ચકાસણી કરો".trim(), null, AlertType.INFO);
                a.setTimeout(Alert.FOREVER);
                display.setCurrent(a);
          }
          os.close();
