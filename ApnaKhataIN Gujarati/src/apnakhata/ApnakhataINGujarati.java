 ...
 * @author Ganesh
 */
public class ApnakhataINGujarati extends MIDlet implements CommandListener, ItemStateListener {
    ...
    private int readRecords(){  
        try {
                ...
                if(rs.listRecordStores() != null) {
                    ...
                    System.out.println("AddBank પર જાઓ અને તમારા બેન્કો પસંદ કરો");
                    return 0;
                }
    ...
    public ApnakhataINGujarati(){    
        ...
        if(check.equalsIgnoreCase("true")) {  //wait
            i = readRecords();            
            cg = new ChoiceGroup("તમારી બેંક પસંદ કરો", ChoiceGroup.EXCLUSIVE);           
            if(i == 1) {
                bankOperation();
            }
            else {
                 aa = new Alert("ચેતવણી","AddBank પર જાઓ અને તમારા બેન્કો પસંદ કરો", null, AlertType.ALARM);
            ...
            Alert a3;
            a3 = new Alert("ચેતવણી", "ApnaKhata સક્રિય કરો", null, AlertType.WARNING);
    ...
    public static String[] split(String splitStr)
    {
     ...
     if (token.length() > 0) {
        temp = token.toString();
         ex.setLength(0);
         ex.append(temp.substring(temp.indexOf('*')+1,temp.lastIndexOf('*')));
         function.addElement("કૉલ કસ્ટમર કેર".trim());
         cn = ex.toString();
         ex.setLength(0);
         token.setLength(0);
         ex.append(temp.substring(temp.indexOf('@')+1,temp.lastIndexOf('@')));
         function.addElement("વેબસાઈટ");    
         wb = ex.toString();
         token.setLength(0);
         ex.setLength(0);
   ...
   public String file(String filename){
     ...
     public class GUI implements CommandListener, ItemStateListener {      
        private ApnakhataINGujarati parent;
        ...
        public GUI(ApnakhataINGujarati parent, String message, String bname) {   
        
        back1 = new Command ("Back", Command.BACK, 1);
        submit1 = new Command("Submit", Command.SCREEN, 1);
        send1 = new Command("Send", Command.OK, 1);
        radioButton = new ChoiceGroup("કોઈપણ વિકલ્પો પસંદ કરો",Choice.EXCLUSIVE);
