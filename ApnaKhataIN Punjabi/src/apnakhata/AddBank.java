...
public class AddBank extends MIDlet implements CommandListener {
    ...
    public AddBank() {
        ...
        if (check.equalsIgnoreCase("true")) {
            cg = new ChoiceGroup("ਤੁਹਾਡਾ ਬਕ ਚੁਣੋ", ChoiceGroup.MULTIPLE);
            bankOperation();
            readRecords();
        } else {
            Alert a3;
            a3 = new Alert("ਚੇਤਾਵਨੀ", "ਉਤਪਾਦ ਸਰਗਰਮ ਕਰੋ, ਕਿਰਪਾ ਕਰਕੇ", null, AlertType.WARNING);
    ...
    public void save() {
        ...
        try {
            ...
            if (cg.getSelectedFlags(picks) > 0) {
                ...
                alr = new Alert("ਜਾਣਕਾਰੀ", "ਨੂੰ ਸੰਭਾਲਿਆ ਡਾਟਾ", null, AlertType.INFO);
                alr.setTimeout(1000);
                display.setCurrent(alr);
                alr.addCommand(exit);
                alr.setCommandListener(this);
            } else {
                a = new Alert("ਚੇਤਾਵਨੀ", "'ਤੇ ਘੱਟੋ ਘੱਟ ਇੱਕ ਬਕ ਚੈੱਕ ਕਰੋ ਜੀ".trim(), null, AlertType.INFO);
                a.setTimeout(Alert.FOREVER);
                display.setCurrent(a);
            }
            os.close();
