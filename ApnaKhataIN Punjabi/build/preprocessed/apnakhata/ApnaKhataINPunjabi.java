 ...
 * @author Ganesh
 */
public class ApnaKhataINPunjabi extends MIDlet implements CommandListener, ItemStateListener {
    ...
    public ApnaKhataINPunjabi() {
        ...
        if (check.equalsIgnoreCase("true")) {  //wait
            i = readRecords();
            cg = new ChoiceGroup("ਤੁਹਾਡਾ ਬਕ ਚੁਣੋ", ChoiceGroup.EXCLUSIVE);
            if (i == 1) {
                bankOperation();
            } else {
                aa = new Alert("ਚੇਤਾਵਨੀ", "AddBank ਲਈ ਜਾਓ ਅਤੇ ਤੁਹਾਡੇ ਬਕ ਦੀ ਚੋਣ ਕਰੋ ਜੀ", null, AlertType.ALARM);
            ...
            Alert a3;
            a3 = new Alert("ਚੇਤਾਵਨੀ", "ApnaKhata ਸਰਗਰਮ ਕਰੋ ਜੀ", null, AlertType.WARNING);
    ...
    public static String[] split(String splitStr) {
        ...
        if (token.length() > 0) {
            ...
            ex.setLength(0);
            ex.append(temp.substring(temp.indexOf('*') + 1, temp.lastIndexOf('*')));
            function.addElement("ਗਾਹਕ ਕਾਲ ਕਰੋ".trim());
            cn = ex.toString();
            ex.setLength(0);
            token.setLength(0);
            ex.append(temp.substring(temp.indexOf('@') + 1, temp.lastIndexOf('@')));
            function.addElement("ਵੈੱਬਸਾਇਟ");
            wb = ex.toString();
            token.setLength(0);
            ex.setLength(0);
    ...
    public class GUI implements CommandListener, ItemStateListener {
        ...
        private ApnaKhataINPunjabi parent;
        ...
        public GUI(ApnaKhataINPunjabi parent, String message, String bname) {

            back1 = new Command("Back", Command.BACK, 1);
            submit1 = new Command("Submit", Command.SCREEN, 1);
            send1 = new Command("Send", Command.OK, 1);
            radioButton = new ChoiceGroup("ਕਿਸੇ ਵੀ ਚੋਣ ਨੂੰ ਚੁਣੋ", Choice.EXCLUSIVE);
        ...
        public void sms_page(String msg, int len) {
            ...
            if (len != 0) {
                ...
                alert2 = new Alert("SMS ਖਰਚੇ ਮੋਬਾਈਲ ਆਪਰੇਟਰ ਦੁਆਰਾ ਵਰਤੇ ਗਏ ਹਨ", data, null, null);
            ...
            if (c == back1) {
             ...
             ex.printStackTrace();
                ...
                for (int i = 0; i < count1; i++) {
                    ...
                    alert1 = new Alert("SMS ਖਰਚੇ ਮੋਬਾਈਲ ਆਪਰੇਟਰ ਦੁਆਰਾ ਵਰਤੇ ਗਏ ਹਨ", data, null, null);
                }
                ...
                int k = send_sms();
                if (k == 1) {
                    smsSent = new Alert("ਜਾਣਕਾਰੀ ", "ਸੁਨੇਹੇ ਨੂੰ ਸਫਲਤਾਪੂਰਕ ਭੇਜਿਆ ਗਿਆ...", null, AlertType.INFO);
                    ...
                    Alert aa = new Alert("ਚੇਤਾਵਨੀ", "ਸੁਨੇਹਾ ਭੇਜਣ ਲਈ ਅਸਫਲ ਹੈ", null, AlertType.WARNING);
                    aa.setTimeout(Alert.FOREVER);
                    aa.addCommand(exit);
                    aa.setCommandListener(this);
