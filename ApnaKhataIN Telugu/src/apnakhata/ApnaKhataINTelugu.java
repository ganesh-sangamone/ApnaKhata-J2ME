 ...
 * @author Ganesh
 */
public class ApnaKhataINTelugu extends MIDlet implements CommandListener, ItemStateListener{
 private Form form;
    ...
    private ApnaKhataINTelugu.GUI child;
    ...
    public ApnaKhataINTelugu(){    
        ...
        if(check.equalsIgnoreCase("true")) {  //wait
            i = readRecords();            
            cg = new ChoiceGroup("మీ బ్యాంకు ఎంచుకోండి", ChoiceGroup.EXCLUSIVE);           
            if(i == 1) {
                bankOperation();
            }
            else {
                 aa = new Alert("హెచ్చరిక ","AddBank వెళ్లి మీ బ్యాంకులు దయచేసి ఎంచుకోండి", null, AlertType.ALARM);
            ...
            Alert a3;
            a3 = new Alert("హెచ్చరిక", "ApnaKhata క్రియాశీలం చేయండి", null, AlertType.WARNING);
       ...
       if (c == submit)
        {
            ...
            }child = new ApnaKhataINTelugu.GUI(this,message.trim(),cg.getString(cg.getSelectedIndex()));           
        }        
        else if (c == exit)
        {
            try {
    ...
    public static String[] split(String splitStr)
    {
     ...
     if (token.length() > 0) {
        temp = token.toString();
         ex.setLength(0);
         ex.append(temp.substring(temp.indexOf('*')+1,temp.lastIndexOf('*')));
         function.addElement("కస్టమర్ కాల్".trim());
         cn = ex.toString();
         ex.setLength(0);
         token.setLength(0);
         ex.append(temp.substring(temp.indexOf('@')+1,temp.lastIndexOf('@')));
         function.addElement("వెబ్సైట్");    
         wb = ex.toString();
         token.setLength(0);
         ex.setLength(0);
   ...
   public String file(String filename){
     ...
     public class GUI implements CommandListener, ItemStateListener {      
        private ApnaKhataINTelugu parent;
        ...
        public GUI(ApnaKhataINTelugu parent, String message, String bname) {   
        
        back1 = new Command ("Back", Command.BACK, 1);
        submit1 = new Command("Submit", Command.SCREEN, 1);
        send1 = new Command("Send", Command.OK, 1);
        radioButton = new ChoiceGroup("ఏ ఎంపికలు ఎంచుకోండి",Choice.EXCLUSIVE);
        ...
        Display.getDisplay(parent).setCurrent(form3);
            ...
            System.out.println("Bypass data "+data);
              alert2 = new Alert("SMS ఆరోపణలు మొబైల్ ఆపరేటర్లు ద్వారా ఉపయోగిస్తారు", data, null, null);
      ...
      /*  else if(c == call){       
            ...
            for(int i=0; i<count1;i++) {
                ...
                alert1 = new Alert("SMS ఆరోపణలు మొబైల్ ఆపరేటర్లు ద్వారా ఉపయోగిస్తారు", data, null, null);                
            }
            ...
            int k = send_sms();
            if(k == 1) {
                smsSent = new Alert("సమాచారం ","సందేశం విజయవంతంగా పంపబడింది...", null, AlertType.INFO);
                ...
                Alert aa = new Alert("హెచ్చరిక", "సందేశాన్ని పంపడం సాధ్యం కాలేదు", null, AlertType.WARNING);
                aa.setTimeout(Alert.FOREVER);
                aa.addCommand(exit);
                aa.setCommandListener(this);
