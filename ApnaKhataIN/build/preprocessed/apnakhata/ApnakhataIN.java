 ...
 * @author Ganesh
 */
public class ApnakhataIN extends MIDlet implements CommandListener, ItemStateListener {
    ...
    private int readRecords(){  
        try {
                ...
                if(RecordStore.listRecordStores() != null) {
                    ...
                    System.out.println("please go to AddBank and select your banks");
                    return 0;
                }
    ...
    private void bankOperation() {
       ...
       // System.out.print("Str value"+str);
        ...
        form = new Form("ApnaKhata Universal Banking");        
    ...
    public ApnakhataIN(){    
        ...
        if(check.equalsIgnoreCase("true")) {  //wait
            i = readRecords();            
            cg = new ChoiceGroup("Select your Bank", ChoiceGroup.EXCLUSIVE);           
            if(i == 1) {
                bankOperation();
            }
            else {
                 aa = new Alert("Warning","please go to AddBank and select your banks", null, AlertType.ALARM);
            ...
            Alert a3;
            a3 = new Alert("Warning", "Please Activate the product", null, AlertType.WARNING);
       ...
       if (c == submit)
        {
            ...
            for (int x = 0; x < picks.length; x++)
            {
                if (picks[x])
                {                    
                    message = cg.getString(x)+"\n";                    
                    //child = new GUI(this,message.trim());
                }
            }child = new GUI(this,message.trim());           
        }        
        else if (c == exit)
    ...
    public static String[] split(String splitStr)
    {
     ...
     if (token.length() > 0) {
        temp = token.toString();
         ex.setLength(0);
         ex.append(temp.substring(temp.indexOf('*')+1,temp.lastIndexOf('*')));
         function.addElement("Call CustomerCare");
         cn = ex.toString();
         ex.setLength(0);
         token.setLength(0);
         ex.append(temp.substring(temp.indexOf('@')+1,temp.lastIndexOf('@')));
         function.addElement("Go to Website");    
         wb = ex.toString();
         token.setLength(0);
         ex.setLength(0);
     ...
     public class GUI implements CommandListener, ItemStateListener {      
        ...
        Display.getDisplay(parent).setCurrent(form3);
            ...
            System.out.println("Bypass data "+data);
              alert2 = new Alert("Sms charges will be applied as per mobile operator", data, null, null);
      ...
      /*  else if(c == call){       
            ...
            for(int i=0; i<count1;i++) {
                ...
                alert1 = new Alert("Sms charges will be applied as per mobile operator", data, null, null);                
            }
            ...
            int k = send_sms();
            if(k == 1) {
                smsSent = new Alert("Info ","Message sent successfully...", null, AlertType.INFO);
                ...
                Alert aa = new Alert("Warning", "Message failed to send", null, AlertType.WARNING);
                aa.setTimeout(Alert.FOREVER);
                aa.addCommand(exit);
                aa.setCommandListener(this);
