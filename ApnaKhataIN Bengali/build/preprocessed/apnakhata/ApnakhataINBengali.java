 ...
 * @author Ganesh
 */
public class ApnakhataINBengali extends MIDlet implements CommandListener, ItemStateListener {
    ...
    private int readRecords(){  
        try {
                ...
                if(rs.listRecordStores() != null) {
                    ...
                    System.out.println("AddBank যান এবং আপনার ব্যাংক নির্বাচন করুন");
                    return 0;
                }
    ...
    public ApnakhataINBengali(){    
        ...
        if(check.equalsIgnoreCase("true")) {  //wait
            i = readRecords();            
            cg = new ChoiceGroup("আপনার ব্যাংক নির্বাচন", ChoiceGroup.EXCLUSIVE);           
            if(i == 1) {
                bankOperation();
            }
            else {
                 aa = new Alert("সতর্কীকরণ","AddBank যান এবং আপনার ব্যাংক নির্বাচন করুন", null, AlertType.ALARM);
            ...
            Alert a3;
            a3 = new Alert("সতর্কীকরণ", "ApnaKhata সক্রিয় করুন", null, AlertType.WARNING);
    ...
    public static String[] split(String splitStr)
    {
     ...
     if (token.length() > 0) {
        temp = token.toString();
         ex.setLength(0);
         ex.append(temp.substring(temp.indexOf('*')+1,temp.lastIndexOf('*')));
         function.addElement("কল গ্রাহক পরিচর্যা".trim());
         cn = ex.toString();
         ex.setLength(0);
         token.setLength(0);
         ex.append(temp.substring(temp.indexOf('@')+1,temp.lastIndexOf('@')));
         function.addElement("ওয়েবসাইট");    
         wb = ex.toString();
         token.setLength(0);
         ex.setLength(0);
   ...
   public String file(String filename){
     ...
     public class GUI implements CommandListener, ItemStateListener {      
        private ApnakhataINBengali parent;
        ...
        public GUI(ApnakhataINBengali parent, String message, String bname) {   
        
        back1 = new Command ("Back", Command.BACK, 1);
        submit1 = new Command("Submit", Command.SCREEN, 1);
        send1 = new Command("Send", Command.OK, 1);
        radioButton = new ChoiceGroup("কোনো বিকল্প নির্বাচন করুন",Choice.EXCLUSIVE);
