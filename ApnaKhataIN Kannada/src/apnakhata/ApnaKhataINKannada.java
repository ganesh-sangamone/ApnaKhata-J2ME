 ...
 * @author Ganesh
 */
public class ApnaKhataINKannada extends MIDlet implements CommandListener, ItemStateListener{
 private Form form;
    ...
    public ApnaKhataINKannada(){    
        ...
        if(check.equalsIgnoreCase("true")) {  //wait
            i = readRecords();            
            cg = new ChoiceGroup("ನಿಮ್ಮ ಬ್ಯಾಂಕ್ ಆರಿಸಿ", ChoiceGroup.EXCLUSIVE);           
            if(i == 1) {
                bankOperation();
            }
            else {
                 aa = new Alert("ಎಚ್ಚರಿಕೆ","AddBank ಹೋಗಿ ಮತ್ತು ನಿಮ್ಮ ಬ್ಯಾಂಕುಗಳು ಆಯ್ಕೆ ಮಾಡಿ", null, AlertType.ALARM);
            ...
            Alert a3;
            a3 = new Alert("ಎಚ್ಚರಿಕೆ", "ApnaKhata ಸಕ್ರಿಯಗೊಳಿಸಿ ದಯವಿಟ್ಟು", null, AlertType.WARNING);
    ...
    public static String[] split(String splitStr)
    {
     ...
     if (token.length() > 0) {
        temp = token.toString();
         ex.setLength(0);
         ex.append(temp.substring(temp.indexOf('*')+1,temp.lastIndexOf('*')));
         function.addElement("ಗ್ರಾಹಕ ಕಾಲ್".trim());
         cn = ex.toString();
         ex.setLength(0);
         token.setLength(0);
         ex.append(temp.substring(temp.indexOf('@')+1,temp.lastIndexOf('@')));
         function.addElement("ವೆಬ್ಸೈಟ್");    
         wb = ex.toString();
         token.setLength(0);
         ex.setLength(0);
   ...
   public String file(String filename){
     ...
     public class GUI implements CommandListener, ItemStateListener {      
        private ApnaKhataINKannada parent;
        ...
        public GUI(ApnaKhataINKannada parent, String message, String bname) {   
        
        back1 = new Command ("Back", Command.BACK, 1);
        submit1 = new Command("Submit", Command.SCREEN, 1);
        send1 = new Command("Send", Command.OK, 1);
        radioButton = new ChoiceGroup("ಯಾವುದೇ ಆಯ್ಕೆಗಳನ್ನು ಆರಿಸಿ",Choice.EXCLUSIVE);
        ...
        Display.getDisplay(parent).setCurrent(form3);
            ...
            System.out.println("Bypass data "+data);
              alert2 = new Alert("SMS ಚಾರ್ಜಸ್ ಮೊಬೈಲ್ ಆಯೋಜಕರು ಬಳಸುತ್ತಾರೆ", data, null, null);
      ...
      /*  else if(c == call){       
            ...
            for(int i=0; i<count1;i++) {
                ...
                alert1 = new Alert("SMS ಚಾರ್ಜಸ್ ಮೊಬೈಲ್ ಆಯೋಜಕರು ಬಳಸುತ್ತಾರೆ", data, null, null);                
            }
            ...
            int k = send_sms();
            if(k == 1) {
                smsSent = new Alert("ಮಾಹಿತಿ","ಸಂದೇಶವನ್ನು ಯಶಸ್ವಿಯಾಗಿ ಕಳುಹಿಸಲಾಗಿದೆ...", null, AlertType.INFO);
                ...
                Alert aa = new Alert("ಎಚ್ಚರಿಕೆ", "ಸಂದೇಶವನ್ನು ಕಳುಹಿಸಲು ಸಾಧ್ಯವಾಗಲಿಲ್ಲ", null, AlertType.WARNING);
                aa.setTimeout(Alert.FOREVER);
                aa.addCommand(exit);
                aa.setCommandListener(this);
