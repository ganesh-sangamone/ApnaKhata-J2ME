...
public class AddBank extends MIDlet implements CommandListener {
    ...
    public AddBank(){       
        ...
        if(check.equalsIgnoreCase("true")) {
            cg = new ChoiceGroup("உங்கள் வங்கி தேர்வு", ChoiceGroup.MULTIPLE);           
            bankOperation();
            readRecords();
        }
        else {
            Alert a3;
            a3 = new Alert("எச்சரிக்கை", "தயாரிப்பு செயல்படுத்த, தயவு செய்து", null, AlertType.WARNING);
    ...
    public void save() {
        ...
        try {
            ...
            if(cg.getSelectedFlags(picks) > 0){            
                ...
                alr = new Alert("தகவல்","தரவு சேமிக்கப்படும்", null, AlertType.INFO);
                alr.setTimeout(1000);
                ...
                a = new Alert("எச்சரிக்கை", "குறைந்தது ஒரு வங்கி சரிபார்க்கவும்".trim(), null, AlertType.INFO);
                a.setTimeout(Alert.FOREVER);
                display.setCurrent(a);
          }
          os.close();
